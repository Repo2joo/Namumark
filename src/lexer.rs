use crate::{Compiler, toskens::Tokens};
    ...
    pub fn lex(&mut self) {
        ...
        loop {
            ...
            match curr_string {
                '|' => {
                    self.tokens
                        .push(Tokens::Literal(String::from(literal_temp.clone()))); //literal_tamp를 그대로 넣어버리면 clear시 같이 삭제댐 타입 특성상
                    literal_temp.clear();
                    match self.nexto(1) {
                        '|' => {
                ...
                '=' => {
                    self.tokens
                        .push(Tokens::Literal(String::from(literal_temp.clone())));
                    literal_temp.clear();
                ...
                '[' => {
                    ...
                    match self.nexto(0) {
                        '[' => {
                            self.tokens.push(Tokens::LinkOpen(false));
                            self.inc();
                        }
                        _ => match self.nexto(0) {
                            '*' => {
